...
const promptLabel = "marvelade@ubuntu-portfolio:~$";

function getSocialValue(profile, label, fallback) {
    return (
        profile.socials.find((social) => social.label === label)?.value ||
        fallback
    );
}
...
export default function TerminalApp({ portfolio }) {
    ...
    const commandMap = useMemo(
        () => ({
            ...
            skills: portfolio.skills.map(
                (group) => `${group.group}: ${group.items.join(", ")}`,
            ),
            contact: [
                `Email: ${portfolio.profile.email}`,
                `GitHub: ${getSocialValue(portfolio.profile, "GitHub", "github.com/marvelade7")}`,
                `LinkedIn: ${getSocialValue(portfolio.profile, "LinkedIn", "linkedin.com/in/marvellous-adewuyi-a32070278")}`,
            ],
            ...
            <div className="min-h-full rounded-md border border-white/10 bg-[#330d26] p-4 shadow-inner transition-all duration-200">
                {!bootComplete ? (
                    <div className="whitespace-pre-wrap text-white/85">
                        ...
                        <span
                            className="ml-1 inline-block w-[0.65ch] translate-y-[-1px] border-b-2 border-[#8ff0a4] align-middle motion-safe:animate-pulse"
                            aria-hidden="true"
                        />
                    </div>
                ) : (
                    <>
                        ...
                        >
                            <span className="whitespace-nowrap text-[#8ff0a4]">
                                {promptLabel}
                            </span>
                            <input
                                ref={inputRef}
