...
import { createProject, loadPortfolio } from '../lib/api.js';
import {
  selectActiveWindow,
  useWindowStore,
} from '../store/windowStore.js';
...
function buildWallpaperStyles(color) {
  return [
    ...
    {
      background:
        `radial-gradient(circle at 80% 78%, ${rgba(color, 0.45)}, ${rgba(color, 0.18)} 44%, rgba(18, 9, 16, 0.24) 100%)`,
    },
    {
      background:
        `linear-gradient(135deg, ${rgba(color, 0.52)}, ${rgba(color, 0.22)} 45%, rgba(233, 84, 32, 0.22))`,
    },
  ];
}
...
export default function Desktop() {
  ...
  const [projectDialogCategory, setProjectDialogCategory] = useState(null);
  ...
  const [wallpaperColor, setWallpaperColor] = useState(() => {
    ...
    return window.localStorage.getItem(WALLPAPER_STORAGE_KEY) || DEFAULT_WALLPAPER_COLOR;
  });
  ...
  const dockApps = useMemo(() => dockAppIds.map((id) => appsById[id]), [appsById]);
  ...
  useEffect(() => {
    ...
    windows.forEach((windowItem) => {
      ...
      const usableWidth = Math.max(320, viewportWidth - safeLeft - safeRight);
      const usableHeight = Math.max(280, viewportHeight - safeTop - safeBottom);
  ...
  async function addProject(project) {
    ...
    setPortfolio((current) => ({
      ...
      projects: [
        savedProject,
        ...current.projects.filter((item) => item.slug !== savedProject.slug),
      ],
    }));
    closeProjectDialog();
    openApp('files');
  ...
  function openProjectDialog(category) {
    setContextMenu(null);
    setProjectDialogCategory(typeof category === 'string' ? category : null);
    setProjectDialogOpen(true);
  }

  function closeProjectDialog() {
    setProjectDialogOpen(false);
    setProjectDialogCategory(null);
  }
  ...
  const activeAppName = activeWindow ? appsById[activeWindow.id]?.title : 'Desktop';
  ...
  return (
    ...
    >
      ...
      />
      <div className="absolute inset-0 transition-colors duration-500" style={buildWallpaperStyles(wallpaperColor)[wallpaperMode]} />
      ...
      {projectDialogOpen && (
        <AddProjectDialog
          initialCategory={projectDialogCategory}
          onSave={addProject}
...
function DesktopContextMenu({ x, y, onChangeWallpaper, onNewProject, onOpenTerminal }) {
  const items = [
    { label: 'New Project', icon: FilePlus2, action: onNewProject },
    { label: 'Open Terminal', icon: Terminal, action: onOpenTerminal },
    { label: 'Change Wallpaper', icon: MonitorCog, action: onChangeWallpaper },
  ];
