    ...
    <div className="relative grid h-full bg-[#f6f2ef] text-[#211821] md:grid-cols-[230px_1fr]">
      ...
      <section className="flex min-h-0 flex-col overflow-hidden">
        <Toolbar
          ...
          onAddProject={() => onOpenProjectDialog?.(selectedCategory || undefined)}
        />

        <div className="min-h-0 flex-1 overflow-auto p-5">
          {selectedCategory === null ? (
            <FolderGrid folders={categoryFolders} onOpenCategory={openCategory} />
          ) : (
            <ProjectGrid
...
function Crumb({ active, label }) {
  return (
    <>
      ...
      <span className={`truncate ${active ? 'font-semibold text-[#300A24]' : ''}`}>
        {label}
      </span>
    </>
  );
    ...
    <div className="grid grid-cols-2 gap-x-6 gap-y-7 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      {folders.map((folder) => (
        <button
          ...
          onKeyDown={(event) => {
            if (event.key === 'Enter') onOpenCategory(folder.category);
          }}
    ...
    return (
      <div className="grid h-full place-items-center text-center text-sm text-[#765f70]">
        <div>
          ...
          <div className="mt-3 font-semibold text-[#300A24]">{category}</div>
        </div>
      </div>
    );
  }
...
function ProjectTileIcon({ project }) {
  const color = CATEGORY_COLORS[project.category] || CATEGORY_COLORS['Full Stack'];
      ...
      <article className="flex max-h-full w-full max-w-3xl flex-col overflow-hidden rounded-lg border border-black/30 bg-[#f6f2ef] shadow-window">
        <div className="flex h-10 shrink-0 items-center justify-between border-b border-black/20 bg-[#302b30] px-3 text-white">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <span className="ml-2 truncate text-sm font-semibold">{project.title}</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="grid min-h-0 overflow-auto md:grid-cols-[260px_1fr]">
          <div className="bg-[#eee7e1] p-5">
            {project.thumbnail ? (
              ...
              >
                <Folder size={64} fill="rgba(255,255,255,0.18)" />
              </div>
            )}
          ...
          <div className="p-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span
                className={`rounded-full px-3 py-1 text-xs font-semibold ${
                  STATUS_STYLES[project.status] || STATUS_STYLES['In Progress']
                }`}
              >
                {project.status}
              </span>
            ...
            <h1 className="mt-4 text-2xl font-semibold text-[#300A24]">{project.title}</h1>
            <p className="mt-3 text-sm leading-6 text-[#4d3b46]">{project.description}</p>
...
function normalizeProject(project) {
  ...
  return {
    ...
    repo: project.repo || 'https://github.com/marvelade',
...
function inferCategory(project) {
  ...
  if (stack.includes('express') || stack.includes('mongo')) return 'Full Stack';
  if (stack.includes('api') || type.includes('automation')) return 'APIs';
  if (stack.includes('react') && stack.includes('tailwind')) return 'React + Tailwind';
