...
import { profile as fallbackProfile, projects as fallbackProjects, skills as fallbackSkills } from '../seed/data.js';
...
function projectResponse(document) {
  ...
  delete value.__v;
  delete value.updatedAt;
  const techStack = Array.isArray(value.techStack) && value.techStack.length
    ? value.techStack
  ...
  return {
    ...
    createdAt: value.createdAt || value.createdDate || new Date().toISOString(),
    slug: value.slug,
...
function normalizeTechStack(body) {
  if (Array.isArray(body.techStack) && body.techStack.length) {
    return body.techStack.map((item) => String(item).trim()).filter(Boolean);
  }
...
function normalizeProjectPayload(body) {
  ...
  const category = projectCategories.includes(body.category) ? body.category : 'Full Stack';
  const status = projectStatuses.includes(body.status) ? body.status : 'In Progress';
  const techStack = normalizeTechStack(body);
  const github = String(body.github || body.repo || '').trim();

  return {
    title,
    slug: body.slug ? slugify(body.slug) : `${slugify(title)}-${Date.now().toString().slice(-5)}`,
...
export async function getPortfolio(req, res, next) {
  try {
    ...
    res.json({
      ...
      skills: mongoProfile.skills?.length ? mongoProfile.skills : fallbackSkills,
      projects: projectDocs.length ? projectDocs.map(projectResponse) : fallbackProjects.map(projectResponse),
      source: 'mongo',
    });
  } catch (error) {
    next(error);
...
export async function getProfile(req, res, next) {
  try {
    if (!isMongoReady()) {
      res.json({ profile: fallbackProfile, skills: fallbackSkills, source: 'fallback' });
      return;
    }

    const profileDoc = await Profile.findOne({ username: 'devuser' }).lean();
    const mongoProfile = plain(profileDoc) || fallbackProfile;
    res.json({
      profile: mongoProfile,
      skills: mongoProfile.skills?.length ? mongoProfile.skills : fallbackSkills,
...
export async function getAbout(req, res, next) {
  try {
    ...
    res.json({
      ...
      experience: mongoProfile.experience || fallbackProfile.experience || [],
      education: mongoProfile.education || fallbackProfile.education || [],
      source: profileDoc ? 'mongo' : 'fallback',
    });
  } catch (error) {
    next(error);
...
export async function getSkills(req, res, next) {
  try {
    ...
    res.json({
      skills: (await Skill.find().sort({ category: 1, name: 1 }).lean()).map((skill) => ({
        id: skill._id,
        name: skill.name,
        icon: skill.icon,
        level: skill.level,
...
export async function getProjectById(req, res, next) {
  try {
    ...
    if (!isMongoReady()) {
      const project = [...memoryProjects, ...fallbackProjects].find((item) => item.slug === id || item.id === id);
      if (!project) {
    ...
    const projectDoc = await Project.findById(id).lean().catch(() => null);
    if (!projectDoc) {
      res.status(404).json({ ok: false, message: 'Project not found.' });
      return;
    }
...
export async function getProjectsByCategory(req, res, next) {
  try {
    const category = String(req.params.cat || '').trim().toLowerCase();

    if (!isMongoReady()) {
      const projects = [...memoryProjects, ...fallbackProjects].filter((project) => {
        const projectCategory = String(project.category || project.type || '').toLowerCase();
        return projectCategory === category;
      });

      res.json({ projects: projects.map(projectResponse), source: 'fallback' });
      return;
    }

    const projects = await Project.find().sort({ createdAt: 1 }).lean();
    const filtered = projects.filter((project) => String(project.category || '').toLowerCase() === category);
...
export async function updateProject(req, res, next) {
  try {
    ...
    if (!payload.title || !payload.description) {
      res.status(400).json({ ok: false, message: 'Please provide a title and description.' });
      return;
    }

    if (!isMongoReady()) {
      const memoryIndex = memoryProjects.findIndex((project) => project.slug === id || project.id === id);
      if (memoryIndex !== -1) {
        memoryProjects[memoryIndex] = { ...memoryProjects[memoryIndex], ...payload };
        res.json({ ok: true, project: projectResponse(memoryProjects[memoryIndex]), source: 'memory' });
        return;
      }

      const fallbackIndex = fallbackProjects.findIndex((project) => project.slug === id || project.id === id);
      if (fallbackIndex === -1) {
        res.status(404).json({ ok: false, message: 'Project not found.' });
        return;
      }

      const updatedFallback = { ...fallbackProjects[fallbackIndex], ...payload };
      res.json({ ok: true, project: projectResponse(updatedFallback), source: 'fallback' });
      return;
    }

    const projectDoc = await Project.findByIdAndUpdate(id, payload, { new: true, runValidators: true }).lean();
    if (!projectDoc) {
      res.status(404).json({ ok: false, message: 'Project not found.' });
      return;
    }

    res.json({ ok: true, project: projectResponse(projectDoc), source: 'mongo' });
  } catch (error) {
    if (error.code === 11000) {
      res.status(409).json({ ok: false, message: 'A project with that slug already exists.' });
      return;
    }
    next(error);
  }
  ...
  try {
    ...
    if (!isMongoReady()) {
      ...
      if (memoryIndex !== -1) {
        ...
        res.json({ ok: true, project: projectResponse(removed), source: 'memory' });
        return;
      }
      ...
      res.json({ ok: true, project: projectResponse(fallbackProjects[fallbackIndex]), source: 'fallback' });
      return;
    }
...
export async function getProjects(req, res, next) {
  try {
    if (!isMongoReady()) {
      res.json({ projects: [...memoryProjects, ...fallbackProjects].map(projectResponse), source: 'fallback' });
      return;
    }
...
export async function createContactMessage(req, res, next) {
  try {
    ...
    if (payload.name.length < 2 || payload.message.length < 8 || !/^\S+@\S+\.\S+$/.test(payload.email)) {
      res.status(400).json({
        ok: false,
    ...
    const memoryRecord = { ...payload, id: `memory-${Date.now()}`, createdAt: new Date().toISOString() };
